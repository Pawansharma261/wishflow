    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#6d23d9' }}>
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        <View style={{ marginBottom: 30 }}>
          <Text style={{ fontSize: 32, fontWeight: '900', color: '#ffffff' }}>Hey there! 👋</Text>
          <Text style={{ fontSize: 16, color: 'rgba(255,255,255,0.7)', fontWeight: '600' }}>Your automated assistant is ready.</Text>
        </View>

        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20, gap: 10 }}>
          <TouchableOpacity style={{ flex: 1, backgroundColor: profile.whatsapp_connected ? '#22c55e' : 'rgba(255,255,255,0.1)', padding: 15, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' }}>
          ...
          <TouchableOpacity style={{ flex: 1, backgroundColor: profile.has_instagram ? '#ec4899' : 'rgba(255,255,255,0.1)', padding: 15, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' }}>
        ...
        <View style={{ gap: 15 }}>
          <Link href="/contacts" asChild>
            <TouchableOpacity style={{ backgroundColor: '#ffffff', padding: 20, borderRadius: 25, flexDirection: 'row', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10, elevation: 5 }}>
              <View style={{ backgroundColor: '#eff6ff', padding: 12, borderRadius: 15, marginRight: 15 }}>
                <Users color="#3b82f6" size={24} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ color: '#94a3b8', fontSize: 10, fontWeight: '800', letterSpacing: 1 }}>TOTAL CONTACTS</Text>
                <Text style={{ color: '#0f172a', fontSize: 24, fontWeight: '900' }}>{stats.totalContacts}</Text>
              </View>
              <ChevronRight color="#cbd5e1" size={20} />
            </TouchableOpacity>
          </Link>

          <Link href="/wishes?filter=sent" asChild>
            <TouchableOpacity style={{ backgroundColor: '#ffffff', padding: 20, borderRadius: 25, flexDirection: 'row', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10, elevation: 5 }}>
              <View style={{ backgroundColor: '#f0fdf4', padding: 12, borderRadius: 15, marginRight: 15 }}>
                <Send color="#22c55e" size={24} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ color: '#94a3b8', fontSize: 10, fontWeight: '800', letterSpacing: 1 }}>WISHES SENT</Text>
                <Text style={{ color: '#0f172a', fontSize: 24, fontWeight: '900' }}>{stats.sentWishes}</Text>
              </View>
              <ChevronRight color="#cbd5e1" size={20} />
            </TouchableOpacity>
          </Link>

          <Link href="/wishes?filter=pending" asChild>
            <TouchableOpacity style={{ backgroundColor: '#ffffff', padding: 20, borderRadius: 25, flexDirection: 'row', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10, elevation: 5 }}>
              <View style={{ backgroundColor: '#fff7ed', padding: 12, borderRadius: 15, marginRight: 15 }}>
                <Clock color="#f97316" size={24} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ color: '#94a3b8', fontSize: 10, fontWeight: '800', letterSpacing: 1 }}>PENDING</Text>
                <Text style={{ color: '#0f172a', fontSize: 24, fontWeight: '900' }}>{stats.pendingWishes}</Text>
              </View>
              <ChevronRight color="#cbd5e1" size={20} />
            </TouchableOpacity>
          </Link>
        ...
        {radarEvent && (
          <View style={{ marginTop: 30, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 30, padding: 25, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' }}>
            <Gift color="#ffffff" size={32} />
            <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 10, fontWeight: '800', letterSpacing: 2, marginTop: 15 }}>{radarEvent.type.toUpperCase()}</Text>
            <Text style={{ color: '#ffffff', fontSize: 24, fontWeight: '900', marginTop: 5 }}>{radarEvent.name}</Text>
            <Text style={{ color: 'rgba(255,255,255,0.8)', fontSize: 14, fontWeight: '600', marginTop: 4 }}>
              {radarEvent.daysAway === 0 ? '🎉 TODAY!' : `In ${radarEvent.daysAway} days — ${format(radarEvent.date, 'MMM do')}`}
            </Text>
          </View>
        )}
      </ScrollView>
